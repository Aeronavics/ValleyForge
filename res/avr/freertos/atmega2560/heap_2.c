...
//static unsigned portSHORT  heapSTRUCT_SIZE	= ( sizeof( xBlockLink ) + ( sizeof( xBlockLink ) % portBYTE_ALIGNMENT ) );
...
portLONG lPortHeapAvailable( size_t* pLargest )
{
xBlockLink *pxBlock;
portLONG   total = 0;
size_t     largest = 0;
...
//	vTaskSuspendAll();
	{
		/* Only do the search if we've been initialized */
		if( xHeapHasBeenInitialised == pdTRUE )
		{
			/* Go through the list adding all of the sizes and
			 * looking for the largest piece.  The entry with
			 * a NULL next free block is the last entry, and
			 * the blocksize for that entry is the total
			 * original heap size.
			 */
			pxBlock = xStart.pxNextFreeBlock;
			while( pxBlock->pxNextFreeBlock )
			{
				...
				largest = ( ( pxBlock->xBlockSize > largest ) ? pxBlock->xBlockSize : largest );
				pxBlock = pxBlock->pxNextFreeBlock;
			}
		}
	}
