...
void attachInterrupt(gpio_pin_address interruptNum, void (*userFunc)(void), int mode) {
  if( ( PC_INT[interruptNum.port][interruptNum.pin] >= PCINT_0) ) && ( PC_INT[interruptNum.port][interruptNum.pin] < NUM_BANKS ) 
  {
      if( pc_int_sem[PC_INT[interruptNum.port][interruptNum.pin]].procure() )
    ...
    switch (PC_INT[interruptNum.port][interruptNum.pin]) {
...
/* This was original Arduino code
#else
