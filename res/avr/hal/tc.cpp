...
Tc::~Tc(void)
{
	...
	// TODO - Is this comment below a todo?

	// If an instance goes out of scope and this function is called. Then the control registers must be reconfigured to a NULL state.
...
Tc_command_status Tc_imp::load_timer_value(Tc_value value)
{
	switch (timer_number)
	{
		...
		#if defined (__AVR_AT90CAN128__) || defined (__AVR_ATmega2560__) || defined (__AVR_ATmega328__)
		case TC_2:
		{
			*imp_register_table.TCNT_ADDRESS.as_8bit = value.value.as_8bit;
			return TC_CMD_ACK;
    	...
    	case TC_2 :
    	{
    		switch (channel)
    		{
    			case TC_OC_A :
    			{
    				...
    				#elif defined (__AVR_ATmega2560__) || defined (__AVR_ATmega328__)
...
Tc_command_status Tc_imp::disable_ic_interrupt(Tc_ic_channel channel)
{
	...
	switch (timer_number)
	{
		...
		#if defined (__AVR_ATmega2560__) || defined (__AVR_AT90CAN128__) || defined (__AVR_ATmega328)
		case TC_2:
		{
			return TC_CMD_NAK;
		}
