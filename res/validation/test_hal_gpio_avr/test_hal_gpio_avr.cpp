...
int8_t update_button(uint8_t button_number)
{
	...
	{
		if(state == buttons[button_number]->read();
		int8_t state = ;
...
int main(void)
{
	...
	// Initialising the pins, based on the predefined addresses.
	...
	// Enabling interrupts on button C0
	int8_t status = GPIO_INT_SUCCESS;
	uint8_t error_position = ERROR_POSITION_NO_ERORR;
	
	// TODO - Renable
	for(i = BUTTON_1; i <= BUTTON_7; i++)
	{
	...
	if(status != GPIO_INT_SUCCESS && status != GPIO_INT_ALREADY_DONE)
	{
		 ...
		 * GPIO_INT_OUT_OF_RANGE 	-> 	x x x x 0 0 0 1		(-2)
		 * GPIO_INT_ALREADY_TAKEN 	->	x x x x 0 0 1 0		(-1)
		...
		int8_t error_message = (1<<(2+status)) & 0xf;
		if(error_position != ERROR_POSITION_NO_ERORR)
		{
			error_message |= (error_position << 4);
		}
		display_number_as_binary(error_message, LEDs, NUM_PINS);
	} 
	else
	{
		/* -------------------------------------------------------------------------
		...
		{	
			...
			if(is_button_pressed(BUTTON_0))
			{
				...
				} else
				{
					for(i = BUTTON_1; i <= BUTTON_7; i++)
					{
						buttons[i]->enable_interrupt(GPIO_INT_ANY_EDGE, &on_button_press);
					}
					// TODO - use on the remaining buttons try out the rising and falling edge, low level
					int_on();
				}
			...
			if(!interrupts_on)
			{
				...
				set_LED_state(LED_1, !is_button_down(BUTTON_1));
				if(is_button_released(BUTTON_2))
				{
					set_LED_state(LED_2, GPIO_O_TOGGLE);
				}
			}
