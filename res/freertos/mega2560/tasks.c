...
#if configCHECK_TASK_TAG == 1
    volatile portSHORT		tag;				/*< "This is a TCB" tag */
#endif
} tskTCB;

#define tskTCBID	0xEDDE		/* DEED */
...
/*-----------------------------------------------------------*/
	...
	portCHAR  crlf[3] = "\r\n";
        ...
        vTaskSuspendAll();
		{
			...
			strcat( ( portCHAR * ) pcWriteBuffer, crlf );
			totalHeap = lPortHeapAvailable( (size_t*) &largestChunk );
			strcat( ( portCHAR * ) pcWriteBuffer, "Heap\t" );
			pC = ( portCHAR * ) ( pcWriteBuffer + strlen( (char*) pcWriteBuffer ) );
			itoa( largestChunk, pC, 10 );
			pC += strlen( pC );
			*pC = '\t'; pC++;
			ltoa( totalHeap, pC, 10 );
			strcat( ( portCHAR * ) pcWriteBuffer, crlf );
		}
        xTaskResumeAll();
	}
...
#if configCHECK_TASK_TAG == 1
	if ( pxCurrentTCB->tag != tskTCBID )
	{
		/* Oh shit!  The TCB selected is not kosher!  hang here. */
		while (1) {}
	}
#endif
...
//			sprintf( pcStatusString, ( portCHAR * ) "%s\t\t%c\t%u\t%u\t%u\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber );
			pC = pcStatusString;
			strcpy( pC, (char*) pxNextTCB->pcTaskName );
			pC += strlen( pC );
			*pC = '\t'; pC++;
			*pC = cStatus; pC++;
			*pC = '\t'; pC++;
			utoa( ( unsigned int ) pxNextTCB->uxPriority, pC, 10 );
			pC += strlen( pC );
			*pC = '\t'; pC++;
			utoa( usStackRemaining, pC, 10 );
			pC += strlen( pC );
			*pC = '\t'; pC++;
			utoa( ( unsigned int ) ( unsigned int ) pxNextTCB->uxTCBNumber, pC, 10 );
			pC += strlen( pC );
			*pC = '\r'; pC++;
			*pC = '\n';
